...
UDP_IP = "127.0.0.1"
UDP_PORT = 5005
UPDATE_INTERVAL = 1.0 / 30.0  # Update Blender at up to 60 times per second.
...
def update_simple_interaction(gesture, raw_gesture):
    ...
    if not globals()["grabbed_ball"]:
        if gesture == "CLOSED_FIST" and ball_distance < grab_range:
            ...
            velocities["Ball"] = mathutils.Vector((0, 0, 0))
            globals()["ball_release_frames"] = 0
            print("Grabbed Ball")
...
create_interaction_scene() # Comment this out if you only want to see the gestures.
...
def align_floor_and_objects_to_hand(palm_position):
    ...
    globals()["object_velocities"]["Ball"] = mathutils.Vector((0, 0, 0))
...
def update_hand_pose():
    try:
        ...
        # Comment out this area when looking at object interaction to reduce print statememnts
